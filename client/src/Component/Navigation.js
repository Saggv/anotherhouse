...
import { faBell, faUser, faFeather, faHome } from '@fortawesome/free-solid-svg-icons';
...
import {
  ...
  FetchNotification,
  ToggleOpenMessage,
  ToggleOpenMessageClose,
  ToggleOpenNotification,
  ToggleOpenNotificationClose,
} from '../Action/User';
...
function Navigation() {
  ...
  const { isOpenLogin, token, user, barNotification, openUpdate, dataProfileUpdate, openMessage, openNotification } = useSelector(
    (state) => state.User
  );
  ...
  return (
    <Fragment>
      ...
      {isOpenMessageItem ? <ItemMessage></ItemMessage> : null}
      {openUpdate ? <EditProfileInfor data={dataProfileUpdate}></EditProfileInfor> : null}
    </Fragment>
  );
}
